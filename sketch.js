...
function setup() {
  ...
  canW = windowWidth; // -800
  canH = windowHeight; // -200
  canArea = canH * canW
  ...
  player = createSprite((displayWidth - 900)/2, canH/2, carWidth - 50, carHeight - 50);
  ...
  fuel = new Fuel(((Math.round(random(fuelRandomNum))) * 100)/2, -truckRandomY, canW/12, canH/(34/8), 10, fuelImg);
  
  startButton = createImg('Images/PlayButton.png')
  startButton.position(canW/2.5, canH/2.25)
...
function draw() {
  ...
  fill('black')
  rect((displayWidth - 1200)/2, canH/120, canW * (4/5), canH/32);
  ...
  fill('black');
  strokeWeight(5);
  rect((displayWidth - 1200)/2, canH/20, canW/(6/5), canH/60)

  fill('red');
  strokeWeight(0);
  if(playerHealth > 0)
  rect((displayWidth - 1200)/2, canH/20, (playerHealth/playerMaxHealth) * canW/(6/5), canH/60)

  fill('black');
  strokeWeight(5);
  rect(((displayWidth - 1200)/2) + canW/(24/15), canH/(120/9), canW/(24/5), canH/60)

  fill('Grey');
  strokeWeight(0);
  if(playerFuel > 0)
  rect(((displayWidth - 1200)/2) + canW/(24/15), canH/(120/9), (playerFuel/playerMaxFuel) * canW/(24/5), canH/60)
  
  playerFuel -= 0.5;

  player.addImage(playerImg)
  player.scale = Math.floor(canArea/641482);
...
if(isTouching(player, truck4)){
  ...
  drawSprites();
    ...
    fill('darkred')
    menuRect = rect(0, 0, displayWidth + 200, displayHeight + 200)
    fill('yellow')
    textSize(50)
    titleText = text('Car Crashers', (canW - 300)/2, canH/4)
    textSize(20)
    msText = text('High Score: ' + metersSaved + 'm', (canW - 180)/2, (canH / (4/3)))
    startButton.show()
